...
- Reported directly on GUCY2F: at alignment column 2772 the pipeline reconstructed an independent 1 bp deletion arising separately on each of the functional (complete-ORF) lineages that carry it (Saccopteryx_bilineata, Rousettus_aegyptiacus, Pteropus_alecto, Megaderma_lyra) -- i.e. the same deletion "resurrecting" the gene convergently in several independent lineages, which is almost impossible. The correct reading is that the deletion is ANCESTRAL, preserved in the functional lineages, and lost (via random insertions/substitutions accumulated after pseudogenization) in the mostly-pseudogenized lineages that lack it.
- New rule in `03_alignment_events.py`: complete-ORF lineages are trustworthy witnesses of the functional ancestral sequence, so when `--min-functional-witnesses` (default 2) or more phylogenetically INDEPENDENT functional lineages carry exactly the same indel, that indel's state is pinned PRESENT at their common functional ancestor before parsimony resolves the rest of the tree. "Functional" is the reference-free tip `complete_orf` call from `00_<gene>.orf_status.tsv`; "independent" means the functional carriers are not a single clean clade (their MRCA subtree also contains a definite non-carrier). Implemented as a hard pin in `sankoff()` (`pinned={node_idx: state}`; a pinned node is never reported ambiguous); the ancestor is the carriers' MRCA (`functional_consensus_pin` / `mrca_node` / `subtree_tip_sets`). New `--min-functional-witnesses` flag on `bin/pensieve` and the runner; set to 0 to disable. New audit column `functional_ancestral_indel` in `03_<gene>.alignment_characters.tsv` and a summary line listing the fixed characters.
- Verified on real GUCY2F: column 2772 now reconstructs root=PRESENT (ancestral deletion); the four functional lineages inherit it with no event, and only the residue-carrier lineages get an insertion/restoration event -- the implausible independent-deletion-and-resurrection reconstruction is gone.

### Figure legends are now fixed and complete (identical across genes and alignment modes)
- Requested directly: `--alignment perform` figures showed fewer event-marker / pseudogenization-history categories than `--alignment defined` ones. The plot code is identical for both modes -- the difference was purely data-driven, because the legend only listed categories that happened to occur in a given gene's events (a small synthetic gene shows few; a large real gene shows many). `05_plot_events.R` now draws the FULL fixed legend on every figure (all nine event-marker categories and all seven pseudogenization-history categories, in a fixed order, via `limits`/`drop = FALSE`), so the legend is identical and directly comparable across genes and between perform and defined runs regardless of which categories are present in the data.

### IndelMaP removed entirely
- Requested directly. IndelMaP was an optional, independent indel-aware concordance layer that never supplied authoritative state. It is removed from the pipeline, installation, and documentation: the vendored `external/indelMaP/` tree is deleted; `--indelmap`/`--indelmap-dir` are gone from `bin/pensieve`, the runner, and `02_run_asr_backends.sh` (the runner and ASR backend now accept-and-ignore the old flags for back-compat only); `indelmap_concordance()` and its call are removed from `03_integrate_asr_evidence.py`; `02_prepare_asr_inputs.py` no longer writes `nt_for_indelmap.fasta`; and the IndelMaP clone/`--env-only` wiring, requirements note, README/VALIDATION/INFERENCE_SPEC sections, and Makefile target are removed. No IndelMaP output (`*.indelmap_concordance.tsv`) is produced or copied to `final_results`. The core PAML+Pensieve inference is unchanged by the removal.

### VERSION bumped to 4.5.

## v4.4 - start-codon tip markers, and a MACSE-free `--alignment defined` codon-scan path

### New: every sequence's start-codon (ATG) status is now shown on the tree figures
- Requested directly: check the ORF status of every sequence for the presence/absence of a start ATG codon, and reflect a missing start codon visually on the figures. Step 00 (`00_prune_and_check_orf.py`) already recorded `starts_with_atg` per species in `00_<gene>.orf_status.tsv`; that column is now surfaced in the plot.
- `scripts/05_plot_events.R` gains an `--orf-status` flag. Any species whose `starts_with_atg` is not true is marked with a small red X (`shape = 4`) at the tip of its own terminal branch, on BOTH the pseudogenization tree and the event map, drawn last so it sits on top of the branch. The subtitle notes the count (`red X at tip = no ATG start codon (N)`). Absent/omitted file => no markers, no error (the flag is added to the script's strict known-flags allowlist).
- The runner passes `--orf-status results_00/<gene>/00_<gene>.orf_status.tsv`, and `00_<gene>.orf_status.tsv` is now copied into `final_results/<gene>/important_output/`. Because the start-codon audit is identical in both alignment modes, the marker means the same thing under `--alignment perform` and `--alignment defined`.

### New: `--alignment defined` no longer runs MACSE and detects premature stops directly on the supplied codon alignment
- Requested directly: for `--alignment defined`, do not perform MACSE; instead note each sequence's ORF status (including start codon), break the sequence into codons (as `fold -w 3` would, but only ever on real codon boundaries), remove the terminal stop codon, scan the codons for `TAA|TAG|TGA`, record the alignment position of any internal stop and mask it, then run codeml ASR with `cleandata = 0`, keeping every downstream analysis (indel/ancestral inference) the same.
- `scripts/run_one_gene_00_to_04.sh` now runs MACSE (step 01) only under `--alignment perform`. Under `--alignment defined` step 01 is skipped entirely and `require_diagnostics` no longer requires a MACSE NT alignment; the supplied codon alignment is used directly.
- Real pre-existing bug this exposed and fixes: step 00's `strip_terminal_stop()` removed a sequence's terminal stop by DELETING its last three real characters. For a curated codon alignment (whose real terminal codon is frequently followed by trailing alignment gaps, and where only some species carry a terminal stop at all) this deleted characters from the MIDDLE of a row and shortened only some rows, desynchronising the alignment so step 02's equal-length/codon-frame check failed. Confirmed on real data: PDE6C rows became 2634 vs 2637; CNGB3/GUCY2F similarly mixed. Fixed with a `preserve_width` mode (used for `--alignment defined`) that replaces the terminal stop's three real characters with gaps IN PLACE, so the stop is genuinely gone from every gap-stripped/ORF view while the alignment keeps exactly the columns Pensieve promised never to add or remove.
- New `scan_defined_stops()` in `scripts/02_prepare_asr_inputs.py`: with the alignment length already validated as a multiple of three, each codon is simply a consecutive column triplet, so premature stops are read straight off the canonical alignment (no raw->alignment remap, no MACSE frame-phase bookkeeping). Each species' own terminal codon is skipped; every in-frame `TAA/TAG/TGA` is recorded at its exact alignment columns as an independent premature-stop candidate (the four MACSE-era gates are satisfied by construction and reported as such). These feed the same `mask_paml_stops()`/registry the perform path uses, so the PAML-safe view masks every stop to `NNN`, and step 03 event reconstruction and step 04 ancestral/ORF inference are unchanged. `load_raw_stop_diagnostics()`/`build_stop_registry()` (which read step 01 MACSE outputs) are now called only under `--alignment perform`.
- Verified end-to-end through event reconstruction on `already_aligned_bat_genes` (defined mode): CNGA3, GRK7, GUCA1B, GUCA1C, GUCY2F, PDE6C, PDE6H all complete; the direct codon scan reproduces a ground-truth `fold -w 3` scan exactly (e.g. PDE6C 14/14 internal stops), the PAML-safe alignment contains zero remaining stop codons, and every native column is preserved except the gapped terminal stop. CNGB3 is correctly refused: its supplied alignment is length 2393 (not a multiple of 3), i.e. not a codon-frame alignment, so it needs separate preprocessing before `--alignment defined` (this is the same length%3 contract that predates this change, not a regression).

### New: `scripts/prepare_defined_alignment.py`, an opt-in preprocessor for curated alignments that arrive off codon frame
- Requested directly: make CNGB3 runnable under `--alignment defined`. CNGB3's supplied alignment is 2393 columns (not a multiple of three) and ships a `masking_regions.txt` of per-species assembly-gap intervals. Pensieve's core stays strict (it never reframes a defined alignment itself), so this is a separate, explicit preprocessing step the user runs first.
- The helper does ONLY two mechanical, data-preserving things and refuses anything else: (1) assembly-gap masking -- within each flagged region it rewrites gap characters `-` to `N` (explicit unknown, kept ambiguous under codeml cleandata=0), never touching a real base (reproduces `already_aligned_bat_genes/replace_alignment_gaps.py`); (2) trims leading/trailing all-gap padding columns so the length becomes a codon multiple. Internal all-gap columns are deliberately KEPT -- they are real codon-frame insertion columns, and removing them misframes everything downstream. Empirically verified on CNGB3: trimming only the 8 trailing all-gap columns (2393 -> 2385, a multiple of three) makes the intact-ORF species read as a clean codon frame with no spurious in-frame stops, whereas removing the internal all-gap columns introduced spurious stops in every intact species. If after both steps the length is still not a multiple of three, the residual is genuine out-of-frame real content and the helper refuses rather than trimming real sequence.
- CNGB3 now runs end-to-end under `--alignment defined` after `prepare_defined_alignment.py --in CNGB3-17-20260727.fas --mask-regions masking_regions.txt --out CNGB3.defined_ready.fas` (2385 columns, 4502 assembly-gap positions masked to N). New regression test `test_prepare_defined_alignment_frame_fix_and_masking`.

### Real bug (reported directly on GRK7): a long deletion sharing an all-gap column's edge manufactured a spurious shared-core indel event
- Reported directly: at GRK7 alignment columns 61-66 only `Rhinolophus_ferrumequinum` carries residues (a 62-66 insertion; column 61 is an all-gap column, gap in every species), yet a 61-66 indel event was also reported for `Molossus_molossus`, which has nothing but gaps there.
- Root cause in `decompose_run()` (`scripts/03_alignment_events.py`): a maximal gap run is split into a shared core plus its lineage-specific extension against the other observed runs. The residual was decomposed recursively but the shared CORE was not. On real GRK7 data the all-gap column 61 makes `ferrumequinum` a 1 bp (61,61) run, most species a (61,66) run, and `Molossus` a longer (61,71) run. (61,66) correctly decomposes to (61,61)+(62,66), but (61,71) picked its single largest one-breakpoint sub-run (61,66) as an UNDECOMPOSED core, giving (61,66)+(67,71). `Molossus`'s (61,66) core then matched no other species' segment -- every other species had been split into (61,61)+(62,66) -- so it clustered alone as a spurious `Molossus`-only 61-66 indel character over an all-gap span, and was emitted as an `ambiguous_indel_change` event.
- Fixed: `decompose_run()` now decomposes the shared core recursively, exactly like the residual, so a run resolves to the same atomic segments regardless of which larger run it came from. (61,71) now resolves to (61,61)+(62,66)+(67,71): `Molossus` joins the shared (62,66) character and only its genuine (67,71) extension is its own event; `ferrumequinum`'s 62-66 insertion is unchanged. Interior runs that share NEITHER breakpoint (the documented Miniopterus/Nycteris 661-669 case) still never split the core, so that behaviour is preserved. Verified end-to-end on GRK7 (the spurious `Molossus` 61-66 event is gone; total events 132 -> 129). New regression tests: two `decompose_run` unit cases for the exact GRK7 shape, and `test_long_deletion_does_not_manufacture_a_spurious_shared_core_event` end-to-end.

### New: frameshift insertions and frameshift deletions are now coloured separately on the figures
- Requested directly. `scripts/05_plot_events.R` previously drew every confident frameshift indel in a single "Frameshift indel" colour regardless of direction. Frameshift indels with a confident direction are now split into `frameshift_insertion` (purple, `#8e44ad`, "Frameshift insertion") and `frameshift_deletion` (yellow, `#f1c40f`, "Frameshift deletion"); both stay non-red so they remain visible on top of a red branch, and an `ambiguous_indel_change` still routes to the amber ambiguous-disabling marker as before. Shared in-frame insertion/deletion markers are unchanged.

### New: a distinct "partial" ORF status (incomplete but no pseudogenization evidence), so a truncated CDS is no longer mis-shown as pseudogenized
- Reported directly on GRK7: `Cynopterus_brachyotis` has an incomplete ORF (its only lesion is a 349 bp 3'-terminal deletion; no internal frameshift, no premature stop) but was coloured as a confident disabling event (solid red), as if pseudogenized. Absence of a complete ORF is not evidence of pseudogenization.
- ORF status is now three-way (`04_ancestral_orf_walk.py:orf_check`), applied identically to every tip and every reconstructed ancestral node:
  - **intact** -- a complete ORF: starts with ATG, length a multiple of three, no in-frame premature stop (terminal stop optional). A directly-complete sequence stays intact even if catalogued indels exist (they compensate / are artifacts), preserving the PDE6C behaviour.
  - **disrupted (pseudogenized)** -- positive evidence: an in-frame premature stop, OR an INTERNAL (non-terminal) frameshift indel. ATG/length/terminal stop are all optional here (they can be lost after pseudogenization).
  - **partial** -- incomplete (missing ATG and/or length not a multiple of three and/or truncated) but with NO in-frame premature stop and NO internal frameshift. New `has_internal_frameshift()` detects a frameshift as any gap run strictly inside the sequence's own occupied span whose length is not a multiple of three (leading/trailing gaps are 5'/3' truncation and never count; a codon-frame insertion shows up as its own interior padding gaps, so internal insertions are caught too).
- Terminal-indel handling is now one source of truth. `03_alignment_events.py` writes a per-event `terminal_incompleteness` column (an indel whose affected sequences have no coding residue on one side of it, i.e. a 5'/3' truncation), computed on the final post-merge span. `04_ancestral_orf_walk.py` excludes `terminal_incompleteness` frameshift indels from a branch's confident/ambiguous disabling evidence and adds a `partial` branch transition (recording the excluded events in `terminal_incompleteness_events_on_branch`). A premature stop is always internal and is never excluded.
- Figures (`05_plot_events.R`): a `partial` branch is drawn orange (`#e67e22`, legend "Incomplete; no pseudogenization evidence") with an orange bold tip label, distinct from the amber "uncertain" and the red pseudogenization shades; and a terminal-incompleteness frameshift indel is drawn as a muted "Terminal indel (incompleteness, not disabling)" marker instead of a bold frameshift marker, so a partial branch no longer carries a disabling-looking event. Verified on GRK7: `Cynopterus_brachyotis` is now the single `partial` tip; genuinely pseudogenized tips and intact tips are unchanged. New regression tests: `test_partial_orf_status_distinguishes_truncation_from_pseudogenization` and an updated root-adjacent test (a missing ATG alone is now partial, so it uses a genuine internal premature stop to exercise the substitution-only disruption pathway).

### VERSION bumped to 4.4.

## v4.3 - a real conserved-block boundary miscount that silently dropped a real nucleotide

### Real bug, reported directly by the user (GUCA1C/Desmodus_rotundus): a genuine raw nucleotide silently vanished, misreported as a deletion
- Reported directly: comparing the raw sequence window around raw nt 85-95 for `Desmodus_rotundus` in GUCA1C against the final canonical alignment showed a spurious single-nucleotide deletion (`AATATCCATCT` in the raw data vs `AATAT-CATCT` in the alignment) at a position the raw data plainly shows is not missing anything.
- Root cause, traced precisely through the real pipeline files (`01_GUCA1C.macse_NT.fasta`, `01_GUCA1C.conserved_block_correction.tsv`, `00_GUCA1C.orf_status.tsv`): `Desmodus_rotundus`'s raw CDS does not start in frame (`starts_with_atg=False`; reading it naively produces 12 spurious premature stop codons). MACSE's own initial whole-gene alignment proposed a frame-restoration insertion at the very start of Desmodus's row -- rendered as a literal `N` character, not MACSE's usual `!` partial-codon marker. Pensieve's conserved 5' block quality-gate correctly accepted this correction (amino-acid identity against the complete-ORF consensus improved from 0% to 83.3%; see `01_GUCA1C.conserved_block_correction.tsv`).
- `map_raw_nt_to_aln()` (`scripts/01_run_macse_and_extract_events.py`), used to locate the 90-nucleotide conserved-block boundary, only knew to skip `-` (gaps) and `!` (MACSE's documented placeholder) when counting "real" raw nucleotides. It had no way to recognize that this literal `N` was itself a synthetic MACSE insertion, not a real raw base -- so it counted the `N` as raw nucleotide #1, miscounting everything after it by one. The block boundary landed one raw nucleotide early (at Desmodus's true 89th real raw nucleotide instead of the 90th), and the block/remainder split that follows slices the untouched raw sequence at a fixed offset independent of this miscount -- so Desmodus's real 90th raw nucleotide (a `C`) fell into the gap between "end of block" and "start of remainder" and was silently represented nowhere. The resulting alignment correctly reflects that Desmodus is short one nucleotide there (since it genuinely is, relative to what the pipeline actually retained) and reports it as a deletion event -- but the deletion is a `map_raw_nt_to_aln` counting bug, not a real biological event. `raw_nt_to_alignment_column()` (`scripts/02_prepare_asr_inputs.py`), used to map raw premature-STOP coordinates onto the final canonical alignment, had the identical limitation.
- Fixed: both functions now cross-check each aligned character against the next expected character of the species' own real raw sequence (case-insensitive), rather than trusting a fixed set of marker characters. A `-` is always a gap. Any other character that does not match the next real raw base -- `!`, a synthetic `N`, or any other placeholder MACSE might render -- is correctly excluded from the count, regardless of what specific character it is. A genuine `N` that really is present in the raw input itself is still counted normally, since it will match. `resolve_conserved_block()`'s "did MACSE modify this species' block" check was similarly hardened: it now compares block_region's total length against the guaranteed-exact `block_nt` real-character count, rather than checking only for literal `!`/`-` presence, so a synthetic insertion of any character is detected as a modification even if it happens not to co-occur with an ordinary gap.
- Two new regression tests: `test_map_raw_nt_to_aln_skips_macse_synthetic_ambiguity_code` (direct unit coverage: a synthetic leading `N` is correctly excluded; a genuine `N` actually present in the raw sequence is still counted) and an updated `test_build_stop_registry_rejects_non_contiguous_codon_mapping` reflecting the new function signature.

### Real bug, found while validating the fix above on real CNGA3 data: a naive greedy character match desynchronised permanently when a species had a SECOND synthetic insertion later in the same row
- The single-pass greedy match in the first version of the fix above (advance on match, else skip) correctly handled one synthetic MACSE insertion, but as soon as a species' row had a second, independent one further into the same gene, it had no way to recover once a real character failed to match by coincidence -- it could stall with zero further matches for the rest of the sequence. On real data this blocked GUCA1C outright: `Desmodus_rotundus` has exactly this shape (an insertion at the conserved-block boundary plus a second, unrelated one later in the gene), and greedy matching found no STOP-coordinate mapping anywhere past the first insertion.
- Fixed by replacing the greedy scan with a proper sequence alignment (`difflib.SequenceMatcher`'s longest-matching-block algorithm) between a species' own raw sequence and the non-gap characters of its aligned row, built once per species and reused for every lookup against that row (`build_raw_to_alignment_map()`, in both `01_run_macse_and_extract_events.py` and `02_prepare_asr_inputs.py`). Every `equal` opcode block is, by construction, a genuine, correctly-ordered run of real raw characters, however many separate synthetic insertions sit between them -- there is no greedy desync failure mode.
- New regression test `test_raw_to_alignment_map_survives_multiple_scattered_insertions`, directly reproducing two independent synthetic insertions in the same row.

### Real bug, also found running real CNGA3 data: `realign_block_content()`'s codon-multiple trim step discarded real, already-accepted sequence content
- Inherited unmodified from the pre-v4.2 implementation that translated block candidates to protein before aligning them (where trimming to a multiple of 3 was genuinely required to protect `translate_codon_seq` from a desynchronising partial trailing codon). v4.2's MACSE-based rewrite of this function kept the trim but never needed it -- MACSE aligns raw nucleotides directly and is exactly the tool designed to handle non-codon-multiple input.
- Real bug: a block candidate can legitimately be non-codon-multiple in length because it contains a masked `N` (or more than one) standing in for one of MACSE's own inferred positions, and that `N` is not necessarily trailing -- so trimming from the end silently discarded real, already-accepted raw content whenever the excess length happened to include the tail of the candidate rather than only synthetic characters. Confirmed on real GUCA1C/Desmodus_rotundus data: a masked `N` immediately followed by one real trailing nucleotide were both lost together to a 2-character end-trim.
- Fixed: candidates are no longer pre-trimmed before being handed to MACSE.
- New regression test `test_block_realignment_does_not_trim_non_codon_multiple_candidates`.

### Real bug, found while validating the fixes above end-to-end on real CNGA3 data: a raw STOP could pass every existing gate while still being a coincidental, off-frame textual match, not a real codon
- Reported directly by the user after the first two fixes above initially caused CNGA3 to regress from 0 to 31 "independently validated" premature stops, all attributed to a single species (`Phyllostomus_discolor`) -- directly contradicting the gene's known biology (one early frameshift fully explains everything downstream). Root cause, confirmed by direct translation: a raw STOP can map to three CONTIGUOUS canonical alignment columns that DO spell TAA/TAG/TGA -- passing every existing gate, including `codon_confirmed` -- while those three columns do not start on a real codon boundary of the alignment's own shared codon frame (column 1 = codon 1's first base, column 4 = codon 2's first base, and so on, the same frame `codon_to_aa()`/`aa_alignment()` already assume everywhere else in the pipeline). This happens whenever a species has an earlier synthetic insertion whose own total length happens to be a multiple of 3 (so the existing `frame_shifted_at_stop` mod-3 heuristic calls the frame "restored") but which still shifts which alignment COLUMN each later raw position falls on, because the raw premature-stop scanner (`00_prune_and_check_orf.py`) numbers codons by counting from raw position 1 with no knowledge of any correction. Confirmed directly: CNGA3/`Phyllostomus_discolor`'s raw-scanner-reported "codon 18" mapped to alignment columns 53-55 -- not a codon boundary (52 is not a multiple of 3) -- reading as the last two bases of the alignment's real codon 18 (Valine) plus the first base of its real codon 19 (Arginine): `TAA` purely by coincidence of an off-frame window. `Phyllostomus_discolor`'s own protein, translated in its real, frame-aligned codons, matches its relatives cleanly with no disruption at this position at all.
- Fixed with a fourth, independent gate in `build_stop_registry()`: `codon_frame_aligned`, requiring `(aligned_start - 1) % 3 == 0`. All four gates (`independent`, `contiguous`, `codon_frame_aligned`, `codon_confirmed`) must now pass for `pseudogenizing_event_candidate`. New audit field `codon_frame_aligned`.
- New regression test `test_build_stop_registry_rejects_off_frame_codon_coincidence`, directly reproducing the CNGA3/Phyllostomus_discolor shape (a single synthetic insertion earlier in the row shifting a genuine raw-frame stop's mapped columns off the true codon boundary while its text is unchanged).
- Verified against real data end-to-end (all 8 genes re-run): CNGA3 correctly returns to 0 stop-mask events; the 31 false positives are gone, each specifically excluded for `codon_frame_aligned=False`. GRK7 (which had also regressed, 12->81) now shows 7 validated stops spread across 3 species -- a plausible, legitimate result, unlike the single-species concentration that flagged CNGA3 as suspicious.
