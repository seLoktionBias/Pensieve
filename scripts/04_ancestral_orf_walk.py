...
def iter_nodes(root, order="preorder", reverse=False):
    """reverse=True keeps every node's parent-before-children relationship
    (still a valid preorder) but visits each node's own children right-to-left
    instead of left-to-right. Used only for the exported tip/sequence order
    below: ape::plot.phylo (05_plot_events.R) draws the tree with the FIRST
    tip in normal left-to-right preorder at the BOTTOM of the figure and the
    LAST at the TOP, so a FASTA/order table written in that same normal order
    reads bottom-to-top -- backwards relative to a human reading the file
    top to bottom next to the plot. reverse=True produces the order that
    matches the plot read top to bottom instead.
    """
    if order == "preorder":
        stack = [root]
        while stack:
            n = stack.pop(); yield n
            stack.extend(n.children if reverse else reversed(n.children))
    else:
...
def main():
    ...
    node_order = [n.name for n in iter_nodes(pensieve, reverse=True)]
    ...
    for node in iter_nodes(pensieve):
        ...
        if node.is_tip:
            ...
            chars = list(seq)
            uncertain = False
            stop_fills = []  # (start, end, codon) applied only after every indel gap is written
            for cid, char in characters.items():
                ...
                if char.get("character_class") == "indel":
                    ...
                    if codon not in STOPS:
                        ...
                        stop_fills.append((start, end, codon))
            # Real bug, found by inspecting real GUCY2F output: a stop_mask
            # character's own span is not always a clean, lineage-stable 3bp
            # codon position -- the breakpoint decomposition can merge it with
            # an adjacent, variable-length indel's boundary (every offending
            # character we found had length_mod_3 != 0, confirming this).
            # Applying that fill blindly, in character-table order, let a
            # "stop present" call overwrite columns THIS SAME NODE's own
            # indel reconstruction had already, separately, determined to be
            # a genuine deletion -- fabricating a stop codon inside what the
            # pipeline's own indel evidence says is not real sequence for
            # this ancestor at all. A real deletion cannot simultaneously
            # contain a real stop codon, so indel-determined gap always wins:
            # a stop_mask fill is only written where none of its own columns
            # were already zeroed by an indel gap call for this exact node.
            for start, end, codon in stop_fills:
                span = range(start - 1, min(end, len(chars)))
                if any(chars[i] == "-" for i in span):
                    continue
    ...
    order_rows = []
    sequence_rank = 0
    for phylo_rank, node in enumerate(iter_nodes(pensieve, reverse=True), start=1):
        available = node.name in integrated
        if available:
            sequence_rank += 1
        desc = [n.name for n in iter_nodes(node, reverse=True) if n.is_tip]
        order_rows.append({
            "gene": gene,
    ...
    for node in iter_nodes(pensieve):
        ...
        if parent_hist is True:
            ...
            #
            # Real bug, found by inspecting real PDE6C data directly: Node59
            # had catalogued "confident disabling" indel events on its branch
            # (individually frameshifting, length_mod_3 != 0), and was
            # reported as pseudogenized purely because of that -- despite its
            # OWN reconstructed sequence, gap-stripped and checked directly,
            # being a genuinely complete, in-frame ORF (2583bp, ATG start,
            # real terminal stop, zero internal stops), consistent with every
            # descendant also being intact. The individually-frameshifting
            # events' NET effect on the real sequence was zero (they compensate
            # each other, or -- per the alignment being DNA-level rather than
            # strictly codon-level -- one or more is itself an alignment
            # artifact near a codon boundary, e.g. a spurious 2bp gap at a
            # start codon). Trusting the character-level event catalogue over
            # the actual, directly-checked resulting sequence is backwards:
            # the sequence is ground truth, the event catalogue is evidence
            # about it. child_status == "intact" must win regardless of what
            # events are catalogued on the branch.
            if child_status == "intact":
                child_hist = False
                transition = "intact"
            ...
            # Same real bug as above, for the (more common) case where the
            # branch above was already confidently clean: a catalogued
            # confident-disabling event must not override this node's own,
            # directly-checked, genuinely complete ORF.
            child_hist = False
            transition = "intact"
            transition_evidence = ("no_inferred_disabling_history" if not confident_disabling
                                   else "catalogued_disabling_events_but_child_orf_intact")
